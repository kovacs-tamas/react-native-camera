...
{
    if (self.captureNextFrameScheduled) {
        if (self.discardedFramesAmount < 10) {
            self.discardedFramesAmount++;
            return;
        }
        
        __block UIImage *takenImage = [self imageFromSampleBuffer:sampleBuffer];
        ...
        dispatch_async(dispatch_get_main_queue(), ^(void){
            ...
            UIImageOrientation flippedOrientation = UIImageOrientationLeftMirrored;
            ...
            takenImage = [UIImage imageWithCGImage:takenImage.CGImage scale:takenImage.scale orientation:flippedOrientation];
            takenImage = [RNImageUtils forceUpOrientation:takenImage];
            
            NSData *takenImageData = UIImageJPEGRepresentation(takenImage, 0.9);
            NSString *path = [RNFileSystem generatePathInDirectory:[[RNFileSystem cacheDirectoryPath] stringByAppendingPathComponent:@"Camera"] withExtension:@".jpg"];
            ...
            self.videoResolve(response);
            
        });
        
        self.captureNextFrameScheduled = false;
    }
}
...
- (void)captureVideoFrame:(BOOL)isMirrored resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    if (!self.captureNextFrameScheduled) {
        self.isMirrored = isMirrored;
        self.videoResolve = resolve;
        self.videoReject = reject;
        self.captureNextFrameScheduled = true;
    }
}
