...
export default class Camera extends React.Component<PropsType, StateType> {
  ...
  async recordAsync(options?: RecordingOptions) {
    ...
    async captureVideoFrame(isMirrored: boolean) {
        if (!this._cameraHandle)
            return;
        return await CameraManager.captureVideoFrame(isMirrored, this._cameraHandle);
    }

    async checkCamerasAvailability() {
        if (Platform.OS === 'android') {
            return await NativeModules.RNCameraModule.checkCamerasAvailability();
        }
        else {
            throw new Error('hasBackCamera function is not implemented for iOS');
        }
    }
